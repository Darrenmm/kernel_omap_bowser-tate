...
void smb347_redo_apsd(int enable)
{
	...
	if(g_priv == NULL) {
		printk(KERN_ERR "%s - SMB347 device not ready !", __func__);
		return;
	}
...
int twl6030_vbuspres_irq = -1;
EXPORT_SYMBOL(twl6030_vbuspres_irq);
...
static int summit_smb347_remove(struct i2c_client *client)
{
	...
	/* barrier to ensure twl6030_usb_irq has finished at this point,
	   so that we can nullify g_priv */
	if(twl6030_vbuspres_irq != -1) {
		dev_dbg(priv->dev, "Wait to exit twl6030 vbus press"
			"interrupt routine # %i!",twl6030_vbuspres_irq);
		synchronize_irq(twl6030_vbuspres_irq);
	}
